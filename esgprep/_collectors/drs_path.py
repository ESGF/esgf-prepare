...
class DRSPathCollector(Collector):
    ...
    def __iter__(self):
        ...
        try:
            Print.debug(f"DRSPathCollector: Starting iteration over sources: {self.sources}")
            ...
            for source in self.sources:
                ...
                for root, dirs, filenames in os.walk(str(source), followlinks=True):
                    Print.debug(f"DRSPathCollector: Walking root={root}, dirs={dirs}, filenames={filenames}")
                    ...
                    try:
                        ...
                        Print.debug(f"DRSPathCollector: No version found in path {path}: {e}, skipping")
                        continue
                    ...
                    if len(get_drs(path).parts) == idx: # on en est à la version
                        ...
                        if self.default:
                            ...
                            Print.debug(f"DRSPathCollector: get_versions({dataset_path}) returned: {versions}")
                            if versions:
                                latest_version = versions[-1]
                                Print.debug(f"DRSPathCollector: latest_version={latest_version}, latest_version.name={latest_version.name}")
                                # Add version filter with latest version.
                                self.PathFilter.add(name='version_filter', regex='/{}'.format(latest_version.name))
                            else:
                                Print.debug(f"DRSPathCollector: No versions found for dataset path {dataset_path}")

                        # Remove undesired subdirectories to prune os.walk.
                        original_dirs = dirs.copy()
                        dirs[:] = [d for d in dirs if self.PathFilter('/{}'.format(d))]
                        Print.debug(f"DRSPathCollector: PathFilter applied - original_dirs={original_dirs}, filtered_dirs={dirs}")

                        if self.dataset_parent and self.PathFilter(root):
                            Print.debug(f"DRSPathCollector: dataset_parent mode, processing root={root}")
                    ...
                    Print.debug(f"DRSPathCollector: Processing {len(filenames)} filenames: {filenames}")
                    for filename in sorted(filenames):

                        # Ensure that root path satisfies PathFilter.
                        path_filter_result = self.PathFilter(root)
                        Print.debug(f"DRSPathCollector: PathFilter({root}) = {path_filter_result}")
                        if path_filter_result:

                            # Instantiate DRSPath object.
                            path = Path(root, filename)
                            Print.debug(f"DRSPathCollector: Processing file path: {path}")

                            # Dereference "latest" symlink version.
                            if is_latest_symlink(path):
                                original_path = path
                                path = with_latest_target(path)
                                Print.debug(f"DRSPathCollector: Dereferenced latest symlink {original_path} -> {path}")

                            # Apply file filters on filename.
                            is_file = path.is_file()
                            file_filter_result = self.FileFilter(path.name)
                            Print.debug(f"DRSPathCollector: is_file={is_file}, FileFilter({path.name})={file_filter_result}")
                            if is_file and file_filter_result:
                                # Yield DRSPath object.
                                Print.debug(f"DRSPathCollector: YIELDING file: {path}")
                                yield path
                            else:
                                Print.debug(f"DRSPathCollector: SKIPPING file: {path} (is_file={is_file}, file_filter={file_filter_result})")
