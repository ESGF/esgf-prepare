...
class Post_Test_Folder:
    ...
    def is_there_same_number_of_files_in_d_and_v(self, upgrade_from_latest=True):
        ...
        version_dirs = [Path.joinpath(self.folder, "v" + ver) for ver in self.list_version]
        ...
        for ver_dir in version_dirs:
            ...
            if not data_dir.exists():
                print(f"No data directory '{data_name}' found for version directory '{ver_name}'")
                return False
        ...
        for data_dir in data_dirs:
            ...
            if not ver_dir.exists():
                print(f"No version directory '{ver_name}' found for data directory '{data_name}'")
                return False
        ...
        if not upgrade_from_latest:
            ...
            for ver_dir in version_dirs:
                ...
                if len(ver_files) != len(data_files):
                    print(f"Unequal number of files: {len(ver_files)} in {ver_name}, {len(data_files)} in {data_name}")
                    return False
    ...
    def is_there_symlink_between_latest_and_latest_version(self, upgrade_from_latest=True):
        """
        ...
        if not upgrade_from_latest:
            ...
            if not (latest_path.exists() and latest_path.is_symlink() and last_version_path.exists()):
                return False
        ...
        sorted_version_list = sorted(self.list_version, key=lambda x: int(x), reverse=True)
        ...
        for latest_file in latest_files:
            ...
            if file_version != newest_version_with_file:
                print(f"File {filename} points to version {file_version} but newest is {newest_version_with_file}")
                return False
    ...
    def test(self, upgrade_from_latest=True):
        ...
        print("--- HAS valid mapping between files/d******** and v********         -------")
        ...
        assert self.is_there_symlink_between_latest_and_latest_version(upgrade_from_latest), (
            "HAS NOT valid symlinks between latest and appropriate version files"
        )
        print("--- HAS valid symlinks between latest and appropriate version files  -------")
