...
class Post_Test_Folder:
    ...
    def is_there_same_number_of_files_in_d_and_v(
        self,
    ):  # TODO ... ça passerra pas en cas de mise à jour partielle !
        files_dir = Path.joinpath(self.folder, "files")
        list_dversion_in_files = list(
            files_dir.glob("*")
        )  # each of d******* in files folder
    ...
    def is_there_symlink_between_latest_and_latest_version(self):
        ...
        return (
            ...
            and last_version_path == latest_path.resolve()
        )
    ...
    def test(self):
        ...
        assert self.is_there_same_number_of_files_in_d_and_v(), (
            "Has NOT same number of files/d******** and v********"
        )
        ...
        assert self.is_there_symlink_between_latest_and_latest_version(), (
            "HAS NOT symlink between latest and the latest v******** "
        )
