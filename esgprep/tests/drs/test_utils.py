...
def _create_base_netcdf_file(file_path, model_id, variable_id, time_range, version=1, member_id="r1i1p1f1"):
    """
    ...
    if isinstance(variable_id, int) or (isinstance(variable_id, str) and variable_id.isdigit()):
...
def create_files_different_models(directory, count=3, time_range=(1, 10), variable_id="tas"):
    """
...
def create_files_different_variables(directory, model_id=0, count=3, time_range=(1, 10)):
    """
...
def create_files_different_times(directory, model_id=0, variable_id="tas", count=3):
    ...
    for i, time_range in enumerate(time_ranges):
        time_start, time_end = time_range
        subfolder = directory / f"time_{time_start}_{time_end}"
        subfolder.mkdir(exist_ok=True, parents=True)
...
def create_files_different_versions(directory, model_id=0, variable_id="tas", time_range=(1, 10), count=2):
    """
...
def create_files_different_members(directory, model_id=0, variable_id="tas", time_range=(1, 10), count=3):
    """
    ...
    for i in range(1, count+1):
        member_id = f"r{i}i1p1f1"  # Different member IDs: r1i1p1f1, r2i1p1f1, r3i1p1f1, etc.
...
def create_version_specific_files(directory, version_dirs, model_id=0):
    ...
    for i, time_range in enumerate(time_ranges):
        ...
        time_start, time_end = time_range
        filename = subfolder / f"{PROJECT_ID}_{model_id}_{variable_id}_time{time_start}-{time_end}.nc"
        file_path = _create_base_netcdf_file(
...
def create_sample_netcdf_files(directory, count=3, version=1, time_ranges=None):
    ...
    if time_ranges is None:
        ...
        time_ranges = [(1, 10), (10, 20), (20, 30)] if count == 3 else [(i*10, (i+1)*10) for i in range(count)]
