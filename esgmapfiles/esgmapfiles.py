...
def get_master_ID(attributes, ctx):
    ...
    for facet in facets:
        if facet == 'project':
            dataset_ID = dataset_ID.replace(facet, attributes[facet].lower())  # Do the lower case need to be enforced? Waiting for esgf-devel decision.
...
def file_process(inputs):
    ...
    try:
        ...
        # Deduce master ID from full path and --latest-symlink option
