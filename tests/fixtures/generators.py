"""
Test data generators for creating NetCDF files with various configurations.

This module contains utilities for generating fake NetCDF test data with different
characteristics (variables, time ranges, models, versions, members, etc.).
"""
...
def generate_fake_tracking_id():
    """Generate a fake tracking ID for NetCDF files."""
...
def create_version_specific_files(directory, version_dirs, model_id=0):
    ...
    first_version_files = []
    for i, time_range in enumerate(time_ranges):
        subfolder = Path(version_dirs["v1"]) / f"subfolder_{i}"
        subfolder.mkdir(exist_ok=True, parents=True)
    ...
    for i, time_range in enumerate(time_ranges):
        subfolder = Path(version_dirs["v2"]) / f"subfolder_{i}"
        subfolder.mkdir(exist_ok=True, parents=True)
