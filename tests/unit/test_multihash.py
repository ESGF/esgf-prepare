...
from esgprep._utils.checksum import (
    multihash_hex,
    checksum,
    is_multihash_algo,
    MULTIHASH_ALGOS,
...
class TestMultihash:
    ...
    def test_is_multihash_algo(self):
        ...
        assert is_multihash_algo("sha2-256")
        assert is_multihash_algo("sha2-512")
        assert is_multihash_algo("sha3-256")
        assert is_multihash_algo("sha1")  # sha1 is both standard and multihash

        # Standard-only algorithms (not in multihash)
        assert not is_multihash_algo("sha256")
        assert not is_multihash_algo("md5")
        assert not is_multihash_algo("blake2b")
    ...
    def test_include_filename_option(self):
        ...
        try:
            ...
            checksum_without = checksum(
                temp_file_path, "sha2-256", include_filename=False
            )
